...
var require_remove = __commonJS({
  "node_modules/fs-extra/lib/remove/index.js"(exports2, module2) {
    ...
    function remove(path8, callback) {
      if (fs4.rm) return fs4.rm(path8, { recursive: true, force: true }, callback);
      rimraf(path8, callback);
    }
    function removeSync(path8) {
      if (fs4.rmSync) return fs4.rmSync(path8, { recursive: true, force: true });
      rimraf.sync(path8);
    }
    module2.exports = {
      remove: u(remove),
...
var require_file = __commonJS({
  "node_modules/fs-extra/lib/ensure/file.js"(exports2, module2) {
    ...
    function createFile(file2, callback) {
      function makeFile() {
        fs4.writeFile(file2, "", (err) => {
          if (err) return callback(err);
          callback();
        });
      }
      fs4.stat(file2, (err, stats) => {
...
var require_jsonfile = __commonJS({
  "node_modules/jsonfile/index.js"(exports2, module2) {
    ...
    async function _readFile(file2, options = {}) {
      ...
      let data = await universalify.fromCallback(fs4.readFile)(file2, options);
      data = stripBom(data);
      let obj;
      try {
    ...
    function readFileSync(file2, options = {}) {
      ...
      const shouldThrow = "throws" in options ? options.throws : true;
      try {
        let content = fs4.readFileSync(file2, options);
    ...
    async function _writeFile(file2, obj, options = {}) {
      ...
      await universalify.fromCallback(fs4.writeFile)(file2, str, options);
    }
    var writeFile = universalify.fromPromise(_writeFile);
    function writeFileSync(file2, obj, options = {}) {
      const fs4 = options.fs || _fs;
      const str = stringify(obj, options);
      return fs4.writeFileSync(file2, str, options);
    }
    module2.exports = {
      readFile,
      readFileSync,
  ...
  "node_modules/fs-extra/lib/output-file/index.js"(exports2, module2) {
    ...
    function outputFile(file2, data, encoding, callback) {
      ...
      pathExists(dir, (err, itDoes) => {
        if (err) return callback(err);
        if (itDoes) return fs4.writeFile(file2, data, encoding, callback);
        mkdir.mkdirs(dir, (err2) => {
          if (err2) return callback(err2);
          fs4.writeFile(file2, data, encoding, callback);
        });
      });
    }
    function outputFileSync(file2, ...args) {
      ...
      if (fs4.existsSync(dir)) {
        return fs4.writeFileSync(file2, ...args);
      }
      mkdir.mkdirsSync(dir);
...
`));
      ...
      static reconstructOriginalUrl(options) {
        ...
        return new url_1.URL(`${protocol2}//${hostname}${port}${path8}`);
      }
  ...
  "node_modules/js-yaml/lib/dumper.js"(exports2, module2) {
    ...
    function testImplicitResolving(state2, str) {
      for (let index = 0, length = state2.implicitTypes.length; index < length; index += 1) {
        const type = state2.implicitTypes[index];
        if (type.resolve(str)) {
          return true;
        }
      }
  ...
  "node_modules/electron-updater/out/DownloadedUpdateHelper.js"(exports2) {
    ...
    var DownloadedUpdateHelper = class {
      ...
      get cacheDirForPendingUpdate() {
        return path8.join(this.cacheDir, "pending");
      }
      ...
      async getValidCachedUpdateFile(fileInfo, logger) {
        ...
        const updateFile = path8.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
      ...
      getUpdateInfoFile() {
        return path8.join(this.cacheDirForPendingUpdate, "update-info.json");
      }
    };
    ...
    async function createTempUpdateFile(name, cacheDir, log) {
      let nameCounter = 0;
      let result = path8.join(cacheDir, name);
      for (let i = 0; i < 3; i++) {
        try {
          ...
          result = path8.join(cacheDir, `${nameCounter++}-${name}`);
        }
      }
      return result;
    }
...
var require_AppAdapter = __commonJS({
  "node_modules/electron-updater/out/AppAdapter.js"(exports2) {
    ...
    function getAppCacheDir() {
      ...
      let result;
      if (process.platform === "win32") {
        result = process.env["LOCALAPPDATA"] || path8.join(homedir, "AppData", "Local");
      } else if (process.platform === "darwin") {
        result = path8.join(homedir, "Library", "Caches");
      } else {
        result = process.env["XDG_CACHE_HOME"] || path8.join(homedir, ".cache");
      }
      return result;
    }
  }
  ...
  "node_modules/electron-updater/out/ElectronAppAdapter.js"(exports2) {
    ...
    var ElectronAppAdapter = class {
      ...
      get appUpdateConfigPath() {
        return this.isPackaged ? path8.join(process.resourcesPath, "app-update.yml") : path8.join(this.app.getAppPath(), "dev-app-update.yml");
      }
      get userDataPath() {
  ...
  "node_modules/electron-updater/out/providers/GitHubProvider.js"(exports2) {
    ...
    var GitHubProvider = class extends BaseGitHubProvider {
      ...
      async getLatestVersion() {
        ...
        const feed2 = (0, builder_util_runtime_1.parseXml)(feedXml);
        let latestRelease = feed2.element("entry", false, `No published versions on GitHub`);
        let tag = null;
        try {
          if (this.updater.allowPrerelease) {
            ...
            if (currentChannel === null) {
              ...
              for (const element of feed2.getElements("entry")) {
...
${feedXml}`, "ERR_UPDATER_INVALID_RELEASE_FEED");
        ...
        if (result.releaseNotes == null) {
          result.releaseNotes = computeReleaseNotes(this.updater.currentVersion, this.updater.fullChangelog, feed2, latestRelease);
        }
        return {
          tag,
          ...result
    ...
    function computeReleaseNotes(currentVersion, isFullChangelog, feed2, latestRelease) {
      if (!isFullChangelog) {
      ...
      const releaseNotes = [];
      for (const release of feed2.getElements("entry")) {
        let versionRelease;
        try {
    ...
    var PrivateGitHubProvider = class extends GitHubProvider_1.BaseGitHubProvider {
      ...
      resolveFiles(updateInfo) {
        return (0, Provider_1.getFileList)(updateInfo).map((it) => {
          const name = path8.posix.basename(it.url).replace(/ /g, "-");
  ...
  "node_modules/electron-updater/out/AppUpdater.js"(exports2) {
    ...
    var AppUpdater = class _AppUpdater extends events_1.EventEmitter {
      ...
      async getOrCreateStagingUserId() {
        const file2 = path8.join(this.app.userDataPath, ".updaterId");
        try {
      ...
      async getOrCreateDownloadHelper() {
        ...
        if (result == null) {
          ...
          const cacheDir = path8.join(this.app.baseCachePath, dirName || this.app.name);
          if (logger.debug != null) {
      ...
      async executeDownload(taskOptions) {
        ...
        function getCacheUpdateFileName() {
          ...
          if (urlPath.toLowerCase().endsWith(`.${taskOptions.fileExtension.toLowerCase()}`)) {
            return path8.basename(urlPath);
          } else {
            return path8.basename(taskOptions.fileInfo.info.url);
          }
        }
        ...
        let updateFile = path8.join(cacheDir, updateFileName);
        const packageFile = packageInfo == null ? null : path8.join(cacheDir, `package-${version}${path8.extname(packageInfo.path) || ".7z"}`);
        const done = async (isSaveCache) => {
          ...
          const currentBlockMapFile = path8.join(cacheDir, "current.blockmap");
          if (await (0, fs_extra_1.pathExists)(currentBlockMapFile)) {
            await (0, fs_extra_1.copyFile)(currentBlockMapFile, path8.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
          }
      ...
      async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
        try {
          ...
          const downloadOptions = {
            newUrl: fileInfo.url,
            oldFile: path8.join(this.downloadedUpdateHelper.cacheDir, oldInstallerFileName),
            logger: this._logger,
          ...
          const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
            const blockMapFile = path8.join(cacheDir, "current.blockmap");
  ...
  "node_modules/electron-updater/out/BaseUpdater.js"(exports2) {
    ...
    var BaseUpdater = class extends AppUpdater_1.AppUpdater {
      ...
      sanitizeEnvPath(envPath) {
        return envPath.split(path8.delimiter).filter((dir) => path8.isAbsolute(dir)).join(path8.delimiter);
      }
  ...
  "node_modules/electron-updater/out/AppImageUpdater.js"(exports2) {
    ...
    var AppImageUpdater = class extends BaseUpdater_1.BaseUpdater {
      ...
      doInstall(options) {
        ...
        if (!path8.isAbsolute(appImageFile) || appImageFile.includes("\0")) {
        ...
        let destination;
        const existingBaseName = path8.basename(appImageFile);
        const installerPath = this.installerPath;
        if (installerPath == null) {
          this.dispatchError(new Error("No update filepath provided, can't quit and install"));
          return false;
        }
        if (path8.basename(installerPath) === existingBaseName || !/\d+\.\d+\.\d+/.test(existingBaseName)) {
          destination = appImageFile;
        } else {
          destination = path8.join(path8.dirname(appImageFile), path8.basename(installerPath));
        }
      ...
      async doDownloadUpdate(downloadUpdateOptions) {
        ...
        return this.executeDownload({
          ...
          task: async (destinationFile, downloadOptions) => {
            const cachedUpdateFilePath = path8.join(this.downloadedUpdateHelper.cacheDir, CURRENT_MAC_APP_ZIP_FILE_NAME);
      ...
      return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.execFile)(...preparePowerShellExec(`"Get-AuthenticodeSignature -LiteralPath '${tempUpdateFile}' | ConvertTo-Json -Compress"`, 20 * 1e3), (error, stdout, stderr) => {
          ...
          try {
            ...
            if (data.Status === 0) {
              try {
                const normlaizedUpdateFilePath = path8.normalize(data.Path);
                const normalizedTempUpdateFile = path8.normalize(unescapedTempUpdateFile);
    ...
    var NsisUpdater = class extends BaseUpdater_1.BaseUpdater {
      ...
      doInstall(options) {
        ...
        const callUsingElevation = () => {
          this.spawnLog(path8.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e) => this.dispatchError(e));
        };
      ...
      async differentialDownloadWebPackage(downloadUpdateOptions, packageInfo, packagePath, provider) {
        ...
        try {
          const downloadOptions = {
            ...
            oldFile: path8.join(this.downloadedUpdateHelper.cacheDir, builder_util_runtime_1.CURRENT_APP_PACKAGE_FILE_NAME),
            logger: this._logger,
            newFile: packagePath,
  ...
  "node_modules/electron-updater/out/main.js"(exports2) {
    ...
    function doLoadAutoUpdater() {
      if (process.platform === "win32") {
        ...
        try {
          const identity = path8.join(process.resourcesPath, "package-type");
...
// set/electron/main.ts
var import_electron7 = require("electron");
var import_node_path7 = __toESM(require("node:path"), 1);

// node_modules/@openflow/protocol/index.ts
var WS_PATH = "/ws";
var DEFAULT_PORT = 17800;

// node_modules/@openflow/desktop/dist/apps.js
var APPS = {
  set: {
    name: "set",
    title: "set[flow]",
...
function uiPort(one, env2 = process.env) {
  const own = env2[`OPENFLOW_${one.name.toUpperCase()}_UI_PORT`];
  return Number(own) || (Number(env2.OPENFLOW_PORT_BASE) || 5173) + one.ui;
}

// node_modules/@openflow/desktop/dist/dev.js
function devUrl(one, env2 = process.env) {
  if (env2.OPENFLOW_DEV_URL)
    return env2.OPENFLOW_DEV_URL;
  return env2.OPENFLOW_DEV ? `http://localhost:${uiPort(one, env2)}` : "";
}
function markDev(win) {
...
// node_modules/@openflow/desktop/dist/serve.js
var import_electron = require("electron");
var import_node_fs = __toESM(require("node:fs"), 1);
var import_node_path2 = __toESM(require("node:path"), 1);

// node_modules/@openflow/desktop/dist/within.js
var import_node_path = __toESM(require("node:path"), 1);
function within(root, rel) {
  const file2 = import_node_path.default.join(root, import_node_path.default.normalize(rel));
  if (file2 !== root && !file2.startsWith(root + import_node_path.default.sep))
    return null;
  return file2;
}

// node_modules/@openflow/desktop/dist/mime.js
var TYPES = {
...
var send = async (file2, cors) => {
  try {
    const body = await import_node_fs.default.promises.readFile(file2);
    const headers = {
      "content-type": TYPES[import_node_path2.default.extname(file2)] ?? "application/octet-stream"
    };
    if (cors)
...
function serve(one, dist, mounts = {}) {
  import_electron.protocol.handle(one.name, async (request) => {
    let rel;
    try {
    ...
    for (const [prefix, where] of Object.entries(mounts)) {
      ...
      if (!root)
        return new Response("not found", { status: 404 });
      const file3 = within(root, rel.slice(prefix.length));
      return file3 ? send(file3, true) : new Response("forbidden", { status: 403 });
    }
    if (rel === "/")
      rel = "/index.html";
    const file2 = within(dist, rel);
    return file2 ? send(file2, false) : new Response("forbidden", { status: 403 });
  });
}

// node_modules/@openflow/desktop/dist/state.js
var import_electron2 = require("electron");
var import_node_os = __toESM(require("node:os"), 1);
var import_node_path3 = __toESM(require("node:path"), 1);
function state(one) {
  const home = process.env.OPENFLOW_HOME ?? import_node_path3.default.join(import_node_os.default.homedir(), ".openflow");
  const here = import_node_path3.default.join(home, one.name, "electron");
  import_electron2.app.setPath("userData", here);
  return here;
}

// node_modules/@openflow/desktop/dist/update.js
var import_electron3 = require("electron");
var import_node_fs2 = __toESM(require("node:fs"), 1);
var import_node_path4 = __toESM(require("node:path"), 1);
var import_electron_updater = __toESM(require_main2(), 1);
var EVERY_MS = 6 * 60 * 60 * 1e3;
var feed = () => import_electron3.app.isPackaged && import_node_fs2.default.existsSync(import_node_path4.default.join(process.resourcesPath, "app-update.yml"));
function updates(one) {
  if (!feed())
    return;
  ...
  const ask = () => {
    void import_electron_updater.autoUpdater.checkForUpdates()?.catch(() => {
    });
  };
  ask();
  const again = setInterval(ask, EVERY_MS);
  import_electron3.app.on("before-quit", () => clearInterval(again));
}

// node_modules/@openflow/desktop/dist/window.js
var import_electron6 = require("electron");
var import_node_path6 = __toESM(require("node:path"), 1);

// node_modules/@openflow/desktop/dist/bounds.js
var import_electron4 = require("electron");
var import_node_fs3 = __toESM(require("node:fs"), 1);
var import_node_path5 = __toESM(require("node:path"), 1);
var file = () => import_node_path5.default.join(import_electron4.app.getPath("userData"), "window.json");
function remembered() {
  try {
    const held = JSON.parse(import_node_fs3.default.readFileSync(file(), "utf8"));
...
function remember(win) {
  try {
    import_node_fs3.default.writeFileSync(file(), `${JSON.stringify(win.getNormalBounds(), null, 2)}
`);
  } catch {
  }
}

// node_modules/@openflow/desktop/dist/navigate.js
var import_electron5 = require("electron");
var ours = (url, { home, dev }) => {
  const here = new URL(dev || home);
  const there = new URL(url);
  return here.protocol === "http:" || here.protocol === "https:" ? there.origin === here.origin : there.protocol === here.protocol;
};
function govern(win, policy) {
  win.webContents.setWindowOpenHandler(({ url, features }) => {
    const mine = policy.popup?.(features);
    if (mine)
      return { action: "allow", overrideBrowserWindowOptions: mine };
...
// node_modules/@openflow/desktop/dist/window.js
var RETRY_MS = 1e3;
var preload = () => import_node_path6.default.join(__dirname, "preload.cjs");
function open(spec) {
  const { app: app2, home, dev } = spec;
  const win = new import_electron6.BrowserWindow({
    ...
    title: app2.title,
    backgroundColor: app2.background,
    webPreferences: {
      preload: preload(),
  ...
  win.webContents.on("did-finish-load", () => {
    console.log(`${app2.name}: ${home}${spec.note ? ` \u2014 ${spec.note}` : ""}`);
  });
  win.webContents.on("did-fail-load", (_event, code, why, url, isMainFrame) => {
    console.error(`${app2.name}: could not load ${url} \u2014 ${why} (${code})`);
...
function lifecycle(electron4, reopen) {
  electron4.on("activate", () => {
    if (import_electron6.BrowserWindow.getAllWindows().length === 0)
      reopen();
  });
  electron4.on("window-all-closed", () => electron4.quit());
}

// set/electron/main.ts
var SET = APPS.set;
var DIST = import_node_path7.default.resolve(__dirname, "..", "..", "dist");
var PORT = Number(process.env.OPENFLOW_PORT) || DEFAULT_PORT;
var BRIDGE = process.env.OPENFLOW_BRIDGE_WS ?? `ws://127.0.0.1:${PORT}${WS_PATH}`;
var DEV = devUrl(SET);
var HOME = DEV || `${SET.name}://app/`;
state(SET);
scheme(SET);
var window2 = () => {
  open({
...
lifecycle(import_electron7.app, window2);
