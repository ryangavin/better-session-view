...
| `npm run dev:set` | just set[flow]: its dev server and its window, one command, closing either closes both. `npm run dev:mix` is the same for the other one. `dev:mix` also opens [the reach view](#the-reach-view) |
