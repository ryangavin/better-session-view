...
export function DeckStrip({ deck: d, index, commands, readFrame, theme, params, deckProps }: Pick<MixerViewProps, 'commands' | 'readFrame' | 'theme' | 'params' | 'deckProps'> & { deck: MixerDeck; index: number }) {
  ...
  return <div {...hostProps} className={`play-deck ${hostProps?.className ?? ''}`} style={{ ...hostProps?.style, '--play-stems': d.stems.length, '--deck-ink': theme.decks[d.id]?.ink ?? theme.primary } as CSSProperties}>
        ...
        <MixerSection className="play-performance">
          ...
          <div className="play-deck-loops" role="group" aria-label={`Deck ${index + 1} loop`}>
            ...
            {commands.setDeckTiming && <ContextControls label={`Deck ${index+1} loop and timing settings`} face="···" active={d.slip || !!d.quantize || !!d.launchBeats} title={`Loop and timing settings${d.slip?' · Slip on':''}${d.quantize?` · Q ${d.quantize}`:''}`}>
              ...
              {(['in','out'] as const).map(boundary=><div className="play-context-row" key={boundary}><span>{boundary==='in'?'In':'Out'} boundary</span><ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} ${boundary} earlier`} title="Earlier by the Q division — a whole beat while Sync is on, ⅛ beat with Q off" onClick={()=>commands.adjustLoop?.(d.id,boundary,-(d.quantize || .125))}>−</ButtonFace><ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} ${boundary} later`} title="Later by the Q division — a whole beat while Sync is on, ⅛ beat with Q off" onClick={()=>commands.adjustLoop?.(d.id,boundary,d.quantize || .125)}>+</ButtonFace></div>)}
            </ContextControls>}
          </div>
        ...
        <div className="play-deck-footer"><div className="play-routing-section"><Separator/><div className="play-route"><Toggle on={d.full} label={`Deck ${index + 1} original full mix`} title="Use the original unseparated track instead of stems" onChange={value => commands.setDeck(d.id, 'full', value)} hint="Plays the original recording instead of its separated stems. The stem controls go quiet, since there are no stems to move.">Full</Toggle><Segmented name="" label={`Deck ${index + 1} crossfade assignment`} items={ROUTE} index={d.route} onChange={value => commands.setDeck(d.id, 'route', value)} hint="Which side of the crossfader this deck answers to. Thru takes it off the crossfader altogether, so it plays wherever the crossfader sits." /><Toggle on={d.cue} onChange={value => commands.setDeck(d.id, 'cue', value)} label={`Deck ${index + 1} headphone cue`} title="Headphone monitoring" hint="Sends this deck to the headphones only. It is taken before the fader and the crossfader, so you can hear a deck the audience cannot."><PhonesIcon/></Toggle></div><Separator/></div>
