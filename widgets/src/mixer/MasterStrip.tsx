  ...
  return <div className="play-master-strip" aria-label="Master mixer">
    <b className="play-master-title">MASTER</b>
      <div className="play-actions">
        <div className="play-run-stop"><Toggle on={running} onChange={commands.setRunning} width={62}>{running ? 'Ⅱ Pause' : '▶ Run'}</Toggle>
        ...
        <Toggle label="Quantize launches to next bar" title="Launch timing: next bar or immediate" on={quantized} onChange={commands.setQuantized} width={62}>{quantized ? '1 bar' : 'Now'}</Toggle></div>
        ...
        <div className="play-loop-controls" role="group" aria-label="Global loop">
          <Button width={62} label="Global loop in" title="Mark loop start at the current beat" onPress={commands.loopIn}>In</Button>
