...
export function MasterStrip({ state, commands, readFrame, theme, params, externalTransport }: MixerViewProps) {
  ...
  return <div className="play-master-strip" aria-label="Master mixer">
        ...
        <div className="play-fx-pickers">
          {(['A', 'B'] as const).map(slot => {
            ...
            return <Fragment key={slot}><div className="play-fx-unit" role="group" aria-label={`FX ${slot}`}>
              ...
              <Select label={`FX ${slot} effect`} items={state.effects.map(e => `${slot} · ${e.name}`)} index={state.effects.findIndex(e => e.id === selected)} onChange={i => commands.setEffect(slot, state.effects[i].id)} width={100} /></div>
            ...
            </div>{slot === 'A' && <div className="play-section-divider" aria-hidden="true"/>}</Fragment>;
          })}
        </div>
    ...
    <div className="play-routing-section"><div className="play-section-divider" aria-hidden="true"/><div className="play-monitor-row">
    ...
    <div className="play-crossfader-row"><Slider name="" label="Crossfader" showValue={false} param={CROSS} value={cross} onChange={value => commands.setMaster('cross', value)} orientation="horizontal" length="auto" display={cross === 0 ? 'Center' : `${Math.abs(cross)} ${cross < 0 ? 'A' : 'B'}`} /></div>
    </div>
  </div>;
}
