...
export function Momentary({label, held, disabled, onHold, onTakeover, children}: {
  ...
  return <button className="play-hold" aria-label={label} data-on={held || undefined} disabled={disabled}
    ...
    onClick={e=>{if(e.detail===0 && !down.current){onHold(true);onHold(false);}}}>{children}</button>;
}
