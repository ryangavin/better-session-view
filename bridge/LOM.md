...
[set[flow]'s `docs/device-chain.md`](https://github.com/openflowfm/set/blob/main/docs/device-chain.md) does it. `tools/diag.ts` is the
...
| `value_items` | StringVector | get | The possible values. **Only for quantized parameters**, and **reaching it through Max is a trap.** It arrives as atoms, so a member whose name holds a space (`Low cut`) comes back as *two* — the list reads correctly when joined while being the wrong length, and the length is what an enum indexes by. `str_for_value(min + k)` per member returns exactly one member's text with no such ambiguity, at n calls instead of one, once. `diag param` prints both the joined text and the count for precisely this reason. Push draws an encoder's value text from this property, which is a separate matter — there Live is the reader, not us. |
