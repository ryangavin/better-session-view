...
const home = path.join(root, '.key-experiments');
...
try {
  ...
  run('clang++',['-std=c++11','-O2',path.join(root,'native/keyfinder.cpp'),`-I${prefix}/include`,path.join(prefix,'lib/libkeyfinder.a'),...fftw,'-o',path.join(home,'keyfinder')]);
...
try {
  ...
  run(python,[path.join(root,'experiments/key/essentia_worker.py'),'--version']);
