...
is the effective sample of every musical beat, with raw detector evidence retained
by `src/musical.ts` — so a track opens gridded rather than ruled at 120. With warp on the stems
...
| touching | read |
|---|---|
| repeatable release smoke, Playwright UI, Vitest DSP, native setup or hardware acceptance | [`docs/release-smoke.md`](docs/release-smoke.md) — `e2e/`, `dsp/`, generated fixtures, actions and evidence gates |
...
| the library table, column sorting, or full-credit filtering | [`docs/window.md`](docs/window.md) — `src/listing.ts`, `src/components/Library.tsx`, `src/libraryOverview.ts` |
...
| choosing required musical alignment boundaries and auditioning the proof of concept | [`docs/alignment.md`](docs/alignment.md) — `src/debug/alignment/` |
...
| `src/listing.ts` | the song table: column ordering and full-credit search. Tested |
