...
async function render(kind:string,hz:number,cutoff:number | undefined,toggleAt?:number,impulse=false,changeTo=800) {
  ...
  if(cutoff!==undefined)effect.setHighPass(cutoff);
  ...
  const merger=ctx.createChannelMerger(2);source.connect(merger,0,0);source.connect(effect.input);effect.output.connect(merger,0,1);merger.connect(ctx.destination);source.start(.2);
  const suspended=toggleAt===undefined?null:ctx.suspend(toggleAt).then(()=>{effect.setHighPass(changeTo);return ctx.resume();});
...
try {
  for(const kind of kinds) {
    const lowOff=await render(kind,60,0),lowOn=await render(kind,60,200),highOff=await render(kind,2000,0),highOn=await render(kind,2000,200),legacy=await render(kind,60,undefined);
    ...
    const tail=await render(kind,60,0,undefined,true),changedTail=await render(kind,60,0,.5,true);
    assert(delta(tail[1],changedTail[1],rate) < 1e-6,`${kind}: existing tail changed`);
    const lowCut=await render(kind,60,20),highCut=await render(kind,60,2000);
    assert(rms(lowCut[1])/rms(lowOff[1])>.85,`${kind}: 20 Hz cutoff harms 60 Hz`);
    assert(rms(highCut[1])/rms(lowOff[1])<.005,`${kind}: 2 kHz cutoff fails to cut bass`);
    const toggled=await render(kind,60,200,1);
    ...
    for (const [from,to] of [[0,800],[800,0]]) {
      const switched=await render(kind,60,from,1,false,to);
      const step=switched[1].slice(rate,rate*1.1).reduce((max,v,i,a)=>i?Math.max(max,Math.abs(v-a[i-1])):max,0);
      assert(step<.03 && switched[1].every(Number.isFinite),`${kind}: bypass transition unstable (${step})`);
    }
    rows.push(`${kind}: bass ${(20*Math.log10(low)).toFixed(2)} dB; high ${(20*Math.log10(high)).toFixed(2)} dB; dry unchanged; bypass within 1e-6; tails preserved; cutoff sweep step ${jump.toFixed(5)}; range 20–2000 Hz checked`);
