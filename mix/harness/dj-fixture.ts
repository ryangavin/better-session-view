...
import { measureOverview } from '../src/play/overview.ts';
...
export async function fixture(ctx:BaseAudioContext):Promise<DeckAsset> {
  ...
  const sourceOverviews=Object.fromEntries(await Promise.all(Object.entries(buffers).map(async([id,b])=>[id,await measureOverview(b,map,new AbortController().signal)])));
