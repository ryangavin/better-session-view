...
|---|---|---|
| P0 waveform positioning | **Missing:** [frames.tsx](https://github.com/openflowfm/widgets/blob/main/src/mixer/frames.tsx), `FrameWaveform` L44–76, renders scroll/loop/playhead but no seek gesture; [model.ts](https://github.com/openflowfm/widgets/blob/main/src/mixer/model.ts), `MixerCommands`, has no seek; [voice.ts](../src/play/voice.ts), `seek`, is internal and halts audio. | P7–10; V1/V8. A low-level seek function is not a usable waveform interaction. |
...
| P0 Cue input lifecycle | **Mismatch/coverage gap:** [Toggle.tsx](https://github.com/openflowfm/widgets/blob/main/src/controls/Toggle.tsx) L79–82 handles down/up/leave; no pointer capture, pointercancel, blur or momentary key handlers. [DeckStrip.tsx](https://github.com/openflowfm/widgets/blob/main/src/mixer/DeckStrip.tsx) L59 recognizes held-Cue Play takeover. | P5/U1; V3. Engine supports latch, but physical mouse/touch/keyboard gesture reliability is unverified. Leave cancels when `on` is true; that is not full cancellation coverage. |
