...
import { pinnedOf, type Every } from './pinned.ts';
...
/** The map pinned per beat at a tempo: what playing the map at that tempo was, before it could be pinned any other way. */
const play = (beats: Beats, tempo: number, every: Every = 'beat') => pinnedOf(beats, tempo, [], every);

describe('the boundaries of a pass', () => {
  it('reads each beat at the rate that lands the next beat on its beat', () => {
    const pass = passOf(play(map, 128), 0);
  ...
  it('is a rate of one everywhere for an even map at its own tempo', () => {
    ...
    const pass = passOf(play(even, 128), 0);
    for (const boundary of pass.boundaries) expect(boundary.rate).toBeCloseTo(1, 4);
    expect(straight(play(even, 128))).toBe(true);
    expect(straight(play(even, 127.9))).toBe(false);
    expect(straight(play(map, 128))).toBe(false);
  });
  ...
  it('is still straight with a detector’s scatter on the beats, and not with a drummer’s', () => {
    ...
    expect(straight(play(scattered, 128))).toBe(true);
    const played = { ...even, samples: even.samples.map((s, i) => s + ((i * 7919) % 5 - 2) * 400) };
    expect(straight(play(played, 128))).toBe(false);
  });

  it('plays twice as fast at twice the tempo', () => {
    const pass = passOf(play(evenBeats(RATE, LENGTH, 128, 0), 256), 0);
  ...
  it('starts mid-beat at that beat’s rate, with only the beats ahead', () => {
    ...
    const pass = passOf(play(map, 128), from);
  ...
  it('pinned per section reads the whole record at one speed, with a boundary at the cut and nowhere else', () => {
    const pass = passOf(pinnedOf(map, 128, [0, 20], 'section'), 0);
    // Where the pass starts, 1.1.1 half a second in, and the cut at bar 21.
    expect(pass.boundaries.length).toBe(3);
    expect(pass.boundaries[1].input).toBeCloseTo(seconds(0), 9);
    expect(pass.boundaries[2].input).toBeCloseTo(seconds(80), 9);
    // The first section is entirely at 128, so it plays at rate one; the
    // second averages the 120 that follows, so it has to be read faster.
    expect(pass.boundaries[1].rate).toBeCloseTo(1, 3);
    expect(pass.boundaries[2].rate).toBeGreaterThan(1.01);
    expect(sourceAt(pinnedOf(map, 128, [0, 20], 'section'), 0, 10, false)).toBeCloseTo(sourceAt(play(map, 128), 0, 10, false), 3);
  });

  it('lasts as long as the beats left take at the target tempo', () => {
    const pass = passOf(play(map, 120), 0);
...
describe('where the sound is', () => {
  it('inverts the output time at every beat, before bar 1 and past the last beat', () => {
    ...
    for (const beat of [startBeat, 0, 150.5, 400, 440]) {
      ...
      expect(sourceAt(play(map, 128), 0, elapsed, false)).toBeCloseTo(seconds(beat), 6);
    }
  });

  it('starts from where the pass started', () => {
    const from = seconds(230.5);
    expect(sourceAt(play(map, 128), from, 0, false)).toBeCloseTo(from, 9);
    expect(sourceAt(play(map, 128), from, 1, false)).toBeCloseTo(from + 128 / 120, 3);
  });

  it('loops back to the top of the file, in beats', () => {
    const from = seconds(230.5);
    const pass = passOf(play(map, 128), from);
    const wrapped = sourceAt(play(map, 128), from, pass.length + 1, true);
    expect(wrapped).toBeCloseTo(sourceAt(play(map, 128), 0, 1, false), 6);
    expect(wrapped).toBeGreaterThan(0);
    expect(wrapped).toBeLessThan(2);
  });

  it('holds at the end of the file when not looping', () => {
    const pass = passOf(play(map, 128), 100);
    expect(sourceAt(play(map, 128), 100, pass.length + 1, false)).toBe(SECONDS);
  });

  it('is never before the start, however early the clock reads', () => {
    expect(sourceAt(play(map, 128), 100, -0.2, true)).toBeCloseTo(100, 9);
  });
});
...
describe('a span', () => {
  ...
  it('ends the pass where the section does, not where the file does', () => {
    ...
    const whole = passOf(play(beats, 120), 30);
    const part = passOf(play(beats, 120), 30, span);
  ...
  it('reads the section exactly as the whole file read it', () => {
    const beats = bent();
    const whole = passOf(play(beats, 128), 30);
    const part = passOf(play(beats, 128), 30, span);
  ...
  it('turns round at the end of the section and comes back to its start', () => {
    ...
    expect(sourceAt(play(beats, 120), 30, 20, true, span)).toBeCloseTo(50, 3);
    expect(sourceAt(play(beats, 120), 30, 40, true, span)).toBeCloseTo(40, 3);
    expect(sourceAt(play(beats, 120), 30, 95, true, span)).toBeCloseTo(35, 3);
  });

  it('holds at the end of the section when it is not looping', () => {
    const beats = evenBeats(RATE, LENGTH, 120, 0);
    expect(sourceAt(play(beats, 120), 30, 40, false, span)).toBeCloseTo(60, 3);
  });

  it('follows the map through a section of a record that bends', () => {
    const beats = bent();
    const at = sourceAt(play(beats, 128), 30, 10, true, span);
  ...
  it('is the whole file for a span that outlived the track it was cut on', () => {
    const beats = evenBeats(RATE, LENGTH, 120, 0);
    expect(held(play(beats, 120), { from: 300, to: 400 })).toEqual({ from: 0, to: SECONDS });
    expect(held(play(beats, 120), { from: 60, to: 30 })).toEqual({ from: 0, to: SECONDS });
    expect(held(play(beats, 120), undefined)).toEqual({ from: 0, to: SECONDS });
    expect(held(play(beats, 120), { from: 30, to: 300 })).toEqual({ from: 30, to: SECONDS });
  });
});
