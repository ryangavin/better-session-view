...
it('prints all action labels, logs discovery, and selects a unique pair without opening it',async()=>{
  ...
  const controller=new LaunchkeyController({snapshot:()=>initialMixer(),commands:{}} as unknown as MixerEngine,request,{getItem:()=>JSON.stringify({enabled:false,sysex:false,input:{id:'in',name:input.name},output:{id:'out',name:output.name}}),setItem:()=>{}});
  ...
  fireEvent.click(screen.getByRole('button',{name:'Deck D'}));
  expect(controller.snapshot().focus).toBe(3);expect(screen.getByRole('button',{name:'Deck D'}).getAttribute('aria-pressed')).toBe('true');
