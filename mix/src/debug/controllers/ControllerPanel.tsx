...
function ConnectedPanel({controller}:{controller:LaunchkeyController}) {
  ...
  return <div className="mf-controller">
    ...
    <p role="status">{state.status}</p><p aria-label="Launchkey screen output">Screen output: {controller.displayDiagnostic}</p><p>{state.wheel}</p><p aria-label="Fader button diagnostics">Fader buttons · CC/RGB v2 · {state.faderMode} {state.selector}</p><p aria-label="Last hardware button packet">Last hardware button packet: {controller.lastButtonInput}</p>
