...
export function AudioSettingsModal({mix,mixer,playView,onClose}:{mix:Mix;mixer:MixerEngine;playView:boolean;onClose():void}) {
  ...
  return <Modal title="Audio settings" width={470} onClose={onClose} className="mf-audio-settings" actions={<><Button onPress={()=>void apply()} disabled={busy||mix.decoding}>{busy?'Applying…':'Apply & restart audio'}</Button><Button onPress={onClose}>Done</Button></>}>
