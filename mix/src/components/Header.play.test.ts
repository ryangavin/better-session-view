...
it('exposes an editable tempo when a local leader is playing', () => {
  ...
  const selection=within(view.getByRole('group',{name:'View selection'}));
  const group=within(view.getByRole('group',{name:'Audio'}));
  expect(group.queryAllByRole('radio')).toHaveLength(0);
  expect(selection.getAllByRole('radio')).toHaveLength(2);
  expect(selection.getByRole('radio',{name:'Play'}).getAttribute('aria-checked')).toBe('true');
  ...
  fireEvent.click(selection.getByRole('radio',{name:'Prep'}));
