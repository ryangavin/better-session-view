...
it('loads the dragged sidebar track into the target deck without selecting it in Prep', async () => {
  ...
  const tracks=[track];
  const select=vi.fn();
  const mix={library:{root:'library',tracks},songs:tracks,libraryBrowser:browseLibrary(tracks,'',{artist:null,album:null}),rows:listing(tracks,'artist'),order:'artist',columns:['artist','album','title'],sortBy:vi.fn(),reorderColumn:vi.fn(),query:'',loading:false,importing:false,selected:null,artOf:()=>null,coverOf:()=>null,notes:null,total:1,note:null,noteBad:false,select} as unknown as Mix;
