  ...
  constructor(private ctx: AudioContext, readonly kind: string) {
    ...
    if (kind === 'reverb') {
      [0.0297, 0.0371, 0.0411, 0.0533].forEach(time => {
        ...
        this.input.connect(delay); delay.connect(feedback); feedback.connect(delay); delay.connect(gain); gain.connect(this.tone);
      ...
      this.input.connect(delay); delay.connect(this.tone); this.tone.connect(feedback); feedback.connect(delay);
