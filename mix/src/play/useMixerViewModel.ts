...
export function useMixerViewModel(tracks: readonly Track[], root: string | null, loader = loadDeckAsset) {
  ...
  const commands: MixerCommands = {
    ...
    setEffectParam: (slot, effectId, paramId, value) => setState(s => ({ ...s, effectValues: { ...s.effectValues, [slot]: { ...s.effectValues?.[slot], [effectId]: { ...s.effectValues?.[slot]?.[effectId], [paramId]: value } } } })),
  ...
  return { state: { ...state, effects: EFFECTS }, commands, readFrame, params, load };
}
