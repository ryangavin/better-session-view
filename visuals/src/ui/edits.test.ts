...
describe('changing a node’s mode', () => {
  const rippled = () =>
    wire(
      [
        ...
        { id: 'e', kind: 'effect', op: 'ripple', x: 1, y: 0 },
  ...
  it('cuts the cords the new mode has nowhere to put', () => {
    ...
    const next = setNode(rippled(), 'e', { op: 'posterize' });
  ...
  it('keeps a cord on an inlet the new mode has as well', () => {
    ...
    const bloomed = setNode(rippled(), 'e', { op: 'bloom' });
...
describe('a value set on an inlet', () => {
  const posterized = () =>
    wire(
      [
        { id: 'g', kind: 'source', op: 'plasma', x: 0, y: 0 },
        { id: 'e', kind: 'effect', op: 'ripple', x: 1, y: 0, knobs: { waves: 0.8, depth: 0.2 } },
        { id: 'o', kind: 'out', x: 2, y: 0 },
      ],
      [
        { from: 'g/c', to: 'e/c' },
  ...
  it('keeps the ones the new mode has as well, and drops the rest', () => {
    ...
    const next = setNode(posterized(), 'e', { op: 'shift' });
    ...
    bloomed.nodes[at] = { ...bloomed.nodes[at], op: 'bloom', knobs: { reach: 0.9 } };
